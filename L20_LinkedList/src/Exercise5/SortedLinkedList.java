...
public class SortedLinkedList {
	...
	public int countElementsRecursion() {
		return countElementsRecursion(first);
	}

	public int countElementsRecursion(Node node) {
		int count = 0;
		if (node == null) {
			return 0;
		} else {
			count++;
			count = count + countElementsRecursion(node.next);
		}
		return count;
	}
